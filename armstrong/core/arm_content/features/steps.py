...
@step('Given I have an Image that refers to (.*)')
def given_i_have_an_image_that_refers_to_path(step, path):
    world.image = SorlImage(image=path)

@step('And I have the following thumbnail presets:')
...
@step('When I ask for each preset thumbnail for the image')
...
@step('When I ask for the (.*) thumbnail for the image')
@fudge.patcher.with_patched_object(ThumbnailBackend, 'get_thumbnail', get_thumbnail_mock)
...
@step('And I have a default preset quality of 100')
def and_i_have_a_default_preset_quality_of_100(step):
    world.defaults = world.defaults or {}
    world.defaults['quality'] = 100

@step('And each thumbnail has a quality of 100')
def and_each_thumbnail_has_a_quality_of_100(step):
    for thumbnail in list(world.thumbnails.values()):
        assert thumbnail._kwargs['quality'] == 100

@step('And the thumbnails without specified quality settings have a quality of 100')
...
@step('When I render its (\w+) thumbnail')
@fudge.patcher.with_patched_object(ThumbnailBackend, 'get_thumbnail', get_thumbnail_mock)
